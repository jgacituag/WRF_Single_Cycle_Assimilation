...
from fortran_interpreter import tempered_wloc
            ...
            for alpha in alphas:
                for loc_scale in loc_scales_files:
                    for truth_member in members:
                        ...
                        outtag_check = f"{cfg.get('experiment_tag','full2d_multicycle_v1')}_{date}_temp{temp}_alpha{int(alpha)}_Loc{int(loc_scale)}_True{int(truth_member)}_kind{kind_tag}"
                        outpath_check = os.path.join(paths["outdir"], f"{outtag_check}.npz")
                        if os.path.exists(outpath_check):
                            print(f"[info] output file {outpath_check} already exists, skipping...")
                            continue
                        ...
                        data = np.load(prepared_file)["state_ensemble"]  # [nx,1,nz,nbv,nvar]
                        mask = np.zeros(data.shape[3], dtype=bool); mask[truth_member] = True
                        truth = data[:, :, :, mask, :][:, :, :, 0, :]        # [nx,1,nz,nvar]
                        xf = data[:, :, :, ~mask, :]                         # [nx,1,nz,Ne,nvar]
                        nx, ny, nz, Ne, nvar = xf.shape
                        
                        ox = np.arange(0, nx, den)
                        oy = np.arange(0, ny, den)
                        oz = np.arange(0, nz, den) 
                        ...
                        print(f"Obs spacing: every {den} grid points")
