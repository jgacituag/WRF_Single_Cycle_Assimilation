...
def _fname_single(tag, method, ntemp, alpha_s, lx, ly, lz,
    ...
    return (f"{tag}_{method}_Nt{ntemp:02d}_as{_fmt(alpha_s)}"
            ...
            f"_obs{ox}_{oy}_{oz}_qc{qc}_True{tm:02d}.npz")
...
def _fname_multi(tag, method, ntemp, alpha_s, lx, ly, lz,
    ...
    return (f"{tag}_{method}_Nt{ntemp:02d}_as{_fmt(alpha_s)}"
            f"_Lx{_fmt(lx)}Ly{_fmt(ly)}Lz{_fmt(lz)}"
            f"_str{stride_str}_qc{qc}_True{tm:02d}.npz")
...
def _worker(args):
    ...
    nx, ny, nz = truth.shape[:3]
    Ne = xf.shape[3]

    t0 = time.time()
    core._log(1, f"[truth {tm:02d}] start  Ne={Ne}  domain={nx}x{ny}x{nz}")

    # compute H(x) for whole domain once
    ...
    if obs_mode == "single":
        ...
        # each QC-passing point is its own independent obs set
        obs_sets = []
        for i in range(nx):
            for j in range(ny):
                for k in range(nz):
        ...
        core._log(1, f"[truth {tm:02d}] full_grid: {len(obs_sets)}/{nx*ny*nz} pts pass QC")
        ...
        core._log(1, f"[truth {tm:02d}] {obs_mode}: {len(ix)}/{nx*ny*nz} pts pass QC")

    if not obs_sets:
        core._log(1, f"[truth {tm:02d}] no valid obs, skipping")
        return []
        ...
        for (method, ntemp, alpha_s, lx, ly, lz) in combos:
            ...
            if obs_mode in ("single", "full_grid"):
                fname = _fname_single(tag, method, ntemp, alpha_s,
                                      lx, ly, lz, loc_str, qc, tm)
            else:
                fname = _fname_multi(tag, method, ntemp, alpha_s,
                                     lx, ly, lz, loc_str, qc, tm)

            out_path = os.path.join(outdir, fname)
            if os.path.exists(out_path) and not cfg.get("skip_existing", False):
                pass   # overwrite — default behaviour
            ...
            # diagnostics to save alongside xa
            ...
            save = dict(
                ...
                obs_error=np.asarray(res.get("obs_error",
                                     res.get("obs_error_raw", R0)), np.float32),
                ox=ox_arr, oy=oy_arr, oz=oz_arr,
                truth_member=np.int32(tm),
            )
            # pass through any extra diagnostic arrays specifit to each method
